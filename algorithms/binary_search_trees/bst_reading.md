...
<ul>
  ...
  <li>**Step 2b.** Determine if `full_bst.left_subtree.left_subtree` is balanced. This tree consists of a single node, so the depths of both its subtrees are 0. Hence, this tree is balanced.
  </li>
  <li>**Step 2c.** Determine if `full_bst.left_subtree.right_subtree` is balanced. Once again, this tree is just a single node, so it is balanced.
</ul>
...
<ul>
  ...
  <li>**Step 3b.** Determine if `full_bst.right_subtree.left_subtree` is balanced. This tree is empty, so it is vacuously balanced (i.e., there's nothing there to balance, so we consider it balanced by default).
  </li>
  <li>**Step 3c.** Determine if `full_bst.left_subtree.right_subtree` is balanced. Once again, this tree is just a single node, so it is balanced.
</ul>
